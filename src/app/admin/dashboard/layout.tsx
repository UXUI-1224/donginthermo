...
const nav = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        label: 'Trademarks',
        href: '/admin/dashboard/trademarks',
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  },
  {
    ...
    items: [
      {
        label: 'Designs',
        href: '/admin/dashboard/settings/designs',
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {
        label: 'Basic',
        href: '/admin/dashboard/settings/basic',
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {
        label: 'Admin',
        href: '/admin/dashboard/settings/admin',
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
function Sidebar({ collapsed, onCollapse }: { collapsed: boolean; onCollapse: () => void }) {
  ...
  return (
    <aside className={`hidden md:flex flex-col h-screen bg-white border-r border-gray-100 transition-all duration-200 shrink-0 ${collapsed ? 'w-[60px]' : 'w-[220px]'}`}>
      <div className="flex items-center justify-between px-4 h-14 border-b border-gray-100">
        {!collapsed && <span className="text-[#016cab] font-bold text-sm tracking-wider truncate">DONGINTHERMO</span>}
        <button onClick={onCollapse} className="ml-auto text-gray-400 hover:text-gray-600 transition-colors p-1 rounded">
          {collapsed ? (
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-2 flex flex-col gap-4">
        {nav.map(({ section, items }) => (
          <div key={section}>
            {!collapsed && <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider px-2 mb-1">{section}</p>}
      ...
      <div className="border-t border-gray-100 px-3 py-3 flex items-center justify-between gap-2">
        {!collapsed && (
          <div className="flex items-center gap-2 min-w-0">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
            <span className="text-sm text-gray-600 truncate">Admin</span>
          </div>
        )}
        <button onClick={handleLogout} className="text-gray-400 hover:text-gray-600 transition-colors shrink-0" title="Sign out">
                ...
                <div key={section}>
                  ...
                  <div className="flex flex-col gap-0.5">
                    {items.map(({ label, href, icon }) => {
                      const active = pathname === href
                      return (
                        <Link key={href} href={href} onClick={() => setOpen(false)} className={`flex items-center gap-2.5 px-2 py-2 rounded-md text-sm font-medium transition-colors ${active ? 'bg-blue-50 text-[#016cab]' : 'text-gray-600 hover:bg-gray-50'}`}>
                          <span className={active ? 'text-[#016cab]' : 'text-gray-400'}>{icon}</span>
                          {label}
                        </Link>
                      )
